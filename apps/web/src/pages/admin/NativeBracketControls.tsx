...
export function NativeBracketControls({ planId, entrants, closed = false }: { planId: string; entrants: Array<{ id: string; name: string }>; closed?: boolean }) {
  ...
  return <section className="card"><h3>Championship and consolation</h3>
    ...
    {preview && <div className="section">
      ...
      {preview.replacing && <button className="btn" disabled={closed || busy || !preview.allowed} onClick={() => { if (window.confirm('Remove the unplayed finals? Pool results will stay recorded.')) void act(async () => { await trpc.eventOps.native.reset.mutate({ planId, revisionToken: preview.revisionToken }); setPreview(null); }, 'Unplayed finals removed. Pool orders and attendance can now be corrected.'); }}>Remove unplayed finals</button>}
    </div>}
