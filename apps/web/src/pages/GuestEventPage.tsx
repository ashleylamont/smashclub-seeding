...
export function GuestEvent({ planId }: { planId: string }) {
  ...
  return <div className="ops-page guest-page"><header><span className="ops-eyebrow">PLAYER AREA / SMASH CLUB</span><h1>{publicEvent.data?.plan.name ?? 'Your event'}</h1><p>Find your pool’s stations, see who plays next and report results. No account or linked player profile is needed.</p><a href={poolPath(`/live/${planId}`, selectedPool)}>Live event board →</a><p className="muted">Pool standings, brackets, announcements and prizes are on the live event board.</p></header>
    {redeeming ? <p role="status">Opening your guest pass…</p> : !valid || matches.isError ? <section className="card"><h2>{session && !valid ? 'Your guest pass has expired' : matches.isError ? 'Refresh your guest pass' : 'Scan in to report a score'}</h2><p>Scan the current event QR on the venue screen or ask a TO for a fresh invitation to start matches and report scores. You can keep browsing without a pass.</p>{matches.isError && <p className="banner banner-warning" role="alert">{matches.error.message} If this pass was revoked, scan the current event QR again.</p>}</section> : <p className="guest-pass-expiry">Guest pass · {guestTimeLeft(session!.expiresAt, now)} remaining</p>}
