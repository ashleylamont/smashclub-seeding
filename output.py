...
def _is_glicko_seeding(seeded_players: List[Dict]) -> bool:
    """Return True when the current output payload comes from Glicko-2."""
    return bool(seeded_players) and seeded_players[0].get("score", {}).get("system") == "glicko2"
...
def print_legend(use_glicko: bool = False) -> None:
    """Print the scoring legend."""
    print(f"\n{DIM}Legend:{RESET}")
    if use_glicko:
        print(
            f"  {DIM}Conservative = Rating - (2 × RD)  |  Higher is better  |  "
            f"RD increases with inactivity/rust{RESET}"
        )
    else:
...
def format_seed(seed: int) -> str:
    ...
    if seed <= 3:
        return f"{BOLD}{YELLOW}{seed}{RESET}"
    if seed <= 8:
        return f"{BOLD}{WHITE}{seed}{RESET}"
    return f"{DIM}{seed}{RESET}"
...
def format_score(score_val: str, is_fallback: bool = False) -> str:
    ...
    if is_fallback:
        return f"{YELLOW}{score_val}{RESET}"
...
def format_recent_results(player: Dict, calculator: 'SeedingCalculator') -> str:
    ...
    first_result = player["recent_results"][0]
    if "outcome" in first_result:
        results_parts = []
        for result in player["recent_results"][:3]:
            ...
            outcome_color = GREEN if result["outcome"] == "W" else RED
            opponent = result.get("opponent", "Unknown")
            rating_delta = result.get("post_rating", 0) - result.get("pre_rating", 0)
            delta_color = GREEN if rating_delta >= 0 else RED
            results_parts.append(
                f"{CYAN}{shorthand}{RESET}: {outcome_color}{result['outcome']}{RESET} vs {WHITE}{opponent[:14]}{RESET} "
                f"{delta_color}{rating_delta:+.0f}{RESET} {DIM}{date_str}{RESET}"
            )
...
def _print_glicko_compact_results(seeded_players: List[Dict], calculator: Optional['SeedingCalculator'], verbose: bool) -> None:
    """Render compact detailed output for Glicko-2 seeding."""
    table_data = []
    headers = ["Seed", "Name", "Co", "Conservative", "Rating", "RD", "Vol", "Record", "Events", "Days", "Recent Sets"]
    ...
    for player in seeded_players:
        ...
        if player["has_history"]:
            ...
            if player.get("results"):
                most_recent = max(player["results"], key=lambda result: result.date)
                days_since = (datetime.now() - most_recent.date).days
            else:
                days_since = "-"

            recent_str = format_recent_results(player, calculator) if calculator else ""
            record = f"{score['wins']}-{score['losses']}"
            table_data.append(
                [
                    seed,
                    name,
                    company,
                    format_score(f"{score['conservative_rating']:.1f}"),
                    format_score(f"{score['rating']:.1f}"),
                    f"{WHITE}{score['rd']:.1f}{RESET}",
                    f"{DIM}{score['volatility']:.6f}{RESET}",
                    f"{WHITE}{record}{RESET}",
                    f"{WHITE}{score['num_tournaments']}{RESET}",
                    days_since,
                    recent_str,
                ]
            )
            ...
            table_data.append(
                [
                    ...
                    f"{DIM}0{RESET}",
                    f"{DIM}-{RESET}",
                    f"{DIM}No match history{RESET}",
                ]
            )

    print("\n" + tabulate(table_data, headers=headers, tablefmt="heavy_grid"))
    print(f"\n{DIM}Note: Conservative = Rating - (2 × RD), so higher is better.{RESET}")
    print(f"{DIM}      RD rises during inactivity, so rusty players seed more conservatively.{RESET}")
    print(f"{DIM}      Vol = Glicko-2 volatility; Events = distinct tournaments in match history.{RESET}")



def _print_legacy_compact_results(seeded_players: List[Dict], calculator: Optional['SeedingCalculator'], verbose: bool) -> None:
    """Render compact detailed output for the legacy placement-based system."""
    table_data = []
    if verbose:
    ...
    for player in seeded_players:
        ...
        if player["has_history"]:
            ...
            if verbose:
                ...
                table_data.append(
                    [seed, name, company, f"{base_score:.2f}", h2h_str, format_score(f"{final_score:.2f}"), best, recent, tournaments, days_since, recent_str]
                )
            else:
    ...
    if verbose:
        ...
        if players_with_h2h:
            for player in players_with_h2h:
                ...
                adj_color = GREEN if h2h_adj < 0 else RED

                print(
                    f"  {WHITE}{player['name']:20s}{RESET} {adj_color}{h2h_adj:+.2f}{RESET}  "
...
def print_seeding_results(
    ...
    use_glicko = _is_glicko_seeding(seeded_players)

    if compact and show_details:
        print_header("TOURNAMENT SEEDING RESULTS")
        print_legend(use_glicko=use_glicko)
        if calculator:
            print_tournament_shorthands(calculator)

        if use_glicko:
            _print_glicko_compact_results(seeded_players, calculator, verbose)
        else:
            _print_legacy_compact_results(seeded_players, calculator, verbose)
        return
    ...
    for player in seeded_players:
        ...
        if use_glicko and player.get('query_alias_used'):
            alias_target = player['query_alias_used']
            print(f"  {DIM}Using saved Glicko query alias → {alias_target['name']} [{alias_target['company']}]{RESET}")

        if not show_details:
            continue

        if not player["has_history"]:
            print(f"  {DIM}No tournament history found{RESET}" if not use_glicko else f"  {DIM}No match history found{RESET}")
            continue

        score = player["score"]
        if use_glicko:
            conservative_score = format_score(f"{score['conservative_rating']:.1f}")
            print(f"  📈 Conservative Rating: {conservative_score}")
            print(f"  🎯 Rating: {WHITE}{score['rating']:.1f}{RESET}")
            print(f"  📏 Rating Deviation (RD): {WHITE}{score['rd']:.1f}{RESET} {DIM}(raw {score.get('raw_rd', score['rd']):.1f}){RESET}")
            print(f"  🌪 Volatility: {DIM}{score['volatility']:.6f}{RESET}")
            print(f"  🥊 Record: {WHITE}{score['wins']}-{score['losses']}{RESET}")
            print(f"  🎮 Events: {WHITE}{score['num_tournaments']}{RESET}")
            print(f"  🧪 Main vs Rookie Sets: {WHITE}{score.get('main_match_count', 0)} main / {score.get('rookie_match_count', 0)} rookie{RESET}")

            if "recent_results" in player and player["recent_results"]:
                print(f"  {GREEN}Recent Sets:{RESET}")
                for result in player["recent_results"][:5]:
                    ...
                    delta_color = GREEN if rating_delta >= 0 else RED
                    print(
                        f"     {DIM}[{result['format']}] {RESET}{outcome_color}{result['outcome']}{RESET} vs "
                        f"{WHITE}{result.get('opponent', 'Unknown')}{RESET} - {result['tournament'][:32]} {DIM}({date_str}){RESET}\n"
                        f"        rating {WHITE}{result.get('pre_rating', 0):.1f}{RESET} → {WHITE}{result.get('post_rating', 0):.1f}{RESET} "
                        f"({delta_color}{rating_delta:+.1f}{RESET}), RD {WHITE}{result.get('pre_rd', 0):.1f}{RESET} → {WHITE}{result.get('post_rd', 0):.1f}{RESET}, "
                        f"vol {DIM}{result.get('pre_volatility', 0.0):.6f}{RESET} → {DIM}{result.get('post_volatility', 0.0):.6f}{RESET}"
                    )
        else:
            ...
            if "recent_results" in player and player["recent_results"]:
                ...
                for result in player["recent_results"][:5]:
                    ...
                    date_str = result["date"].strftime("%b %Y")
                    print(
                        f"     {DIM}[{format_label}]{RESET} {format_placement(str(result['placement']))} - "
                        f"{result['tournament'][:40]} {DIM}({date_str}){RESET}"
                    )
...
def print_bracket_matchups(seeded_players: List[Dict]) -> None:
    ...
    if use_glicko:
        print(f"\n{DIM}Note: Glicko prototype seeding is ordered by conservative rating = Rating - (2 × RD).{RESET}")
    else:
